...
struct Comment {
    ...
    string name;     // Name of the person who commented
    string dateTime; // Date and time of the comment
};
struct Post{
   string post;
   string dateTime;
...
private:
Name user;
vector<pair<string, string>>inbox;
int followers;
int following;
int number;

public:
User(string name, int year, string hostel, int phonenumber, string b) {
    user.name = name;
    user.year = year;
    user.hostel = hostel;
    user.branch = b;
...
//     }
// }
void addComment(string username, int postIndex,const std:: string& commentText) {
...
void sendMessage(string recipientName, const std::string& message) {
...
void followUser(string FollowersName){
...
void suggestUsers() {
    ...
    // Prioritize suggestions from the same branch first
    ...
    // If less than 6 suggestions, proceed with nearby branches
    if (suggestions.size() < 6) {
        for (int i = 0; i < distances.size(); i++) {
            if (distances[i] != INF && i != currentBranchIndex) { 
                for (const Name& potentialUser : BranchList[i]) { 
                    if (potentialUser.name != user.name && !isAlreadyFollowing(userIndex, potentialUser.name)) {
                        suggestions.push_back(potentialUser);
                        if (suggestions.size() == 6) break;
                    }
...
refresh(); // Ensure all printed text is displayed
